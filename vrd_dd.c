...
vrd_init(void)
{
	...
	state = register_blkdev(VRD_DEV_MAJOR, VRD_DEV_NAME);
	if(state < 0)
	{
...
static unsigned int
vrd_make_request(struct request_queue *q, struct bio *bio)
{
	vrd_device *pdevice;
	char *pVHDDData;
	char *pBuffer;
...
int 
vrd_open(struct block_device *bdev, fmode_t mode_t)
{
...
void 
vrd_release(struct gendisk *gd, fmode_t mode_t)
{
	printk(KERN_INFO "device release\n");
}

int 
vrd_ioctl(struct block_device *bdev, fmode_t mode_t, unsigned cmd, unsigned long error)
{
	return -ENOTTY;
}
